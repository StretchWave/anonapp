...
void main() {
  group('Red-Team Security & Authorization Audits', () {
    ...
    test(
      'View-once message maintains media and remains accessible without client-side expiration',
      () {
    ...
    test(
      'Message cannot have content or media altered while keeping original id',
      () {
    ...
    test(
      'ErrorHandler properly maps admin authorization failure to PermissionException',
      () {
    ...
    test('Expired disappearing message is recognized as expired', () {
      ...
      final isExpired =
          expiredMessage.expiresAt != null &&
          nowUtc.isAfter(expiredMessage.expiresAt!);
      expect(isExpired, isTrue);
    });

    // ── Attack 7: Block Enforcement on Messaging ─────────────────────
    test(
      'ErrorHandler correctly catches blocked user message insertion refusal',
      () {
