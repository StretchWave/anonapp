...
void main() {
  ...
  final originalAudioBytes = [79, 103, 103, 83, 0, 2, 0, 0, 0, 0, 0, 0]; // OggS mock header
  final originalBase64Audio = base64Encode(originalAudioBytes);

  group('Voice Message Encryption Pipeline', () {
    test('encrypted voice payload uses ENC_AUDIO:v1: format and hides original audio', () async {
    ...
    test('legacy unencrypted voice message is preserved without re-encryption', () async {
    ...
    test('failed voice decryption returns null safely without throwing', () async {
    ...
    test('Message model with audio deserializes and distinguishes audio from image and text', () {
      final json = {
        'id': 'msg-voice-1',
  ...
  group('Voice Message UI Safe State', () {
    testWidgets('VoicePlayer displays unavailable state safely when audio payload is empty or invalid', (tester) async {
    ...
    testWidgets('MessageBubble displays unavailable state when voice mediaData is null', (tester) async {
    ...
    testWidgets('MessageBubble renders VoicePlayer when valid audio is present', (tester) async {
