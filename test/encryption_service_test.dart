...
void main() {
  group('EncryptionService', () {
    ...
    test('generates unique nonces for repeated audio encryptions', () async {
      ...
      final parts1 = enc1.substring(EncryptionService.audioPrefix.length).split(':');
      final parts2 = enc2.substring(EncryptionService.audioPrefix.length).split(':');
    ...
    test('tampered audio ciphertext fails authentication and returns null safely', () async {
      final audioBytes = [10, 20, 30, 40, 50, 60, 70, 80];
      final ciphertext = await service.encryptAudioBytes(audioBytes, conversationId1);
      ...
      final tamperedCipher = cipher.substring(0, cipher.length - 2) + (cipher.endsWith('A') ? 'B' : 'A');
      final tamperedPayload1 = '$prefix$nonce:$tamperedCipher:$mac';
      final res1 = await service.decryptAudioBytes(tamperedPayload1, conversationId1);
      expect(res1, isNull);

      // 2. Tamper MAC authentication tag
      final tamperedMac = mac.substring(0, mac.length - 2) + (mac.endsWith('A') ? 'B' : 'A');
      final tamperedPayload2 = '$prefix$nonce:$cipher:$tamperedMac';
      final res2 = await service.decryptAudioBytes(tamperedPayload2, conversationId1);
      expect(res2, isNull);

      // 3. Tamper Nonce
      final tamperedNonce = nonce.substring(0, nonce.length - 2) + (nonce.endsWith('A') ? 'B' : 'A');
      final tamperedPayload3 = '$prefix$tamperedNonce:$cipher:$mac';
      final res3 = await service.decryptAudioBytes(tamperedPayload3, conversationId1);
      expect(res3, isNull);
    });
    ...
    test('encryptAudio and decryptAudio base64 convenience round-trip', () async {
      // Base64-encoded audio sample
      final originalB64 = base64Encode(List<int>.generate(128, (i) => (i * 3) % 256));

      final encrypted = await service.encryptAudio(originalB64, conversationId1);
      ...
      final decryptedB64 = await service.decryptAudio(encrypted, conversationId1);
      expect(decryptedB64, equals(originalB64));
    });

    test('decryptAudio preserves legacy unencrypted audio (backward compatibility)', () async {
    ...
    test('malformed audio ciphertext returns null safely without throwing', () async {
      expect(await service.decryptAudioBytes('ENC_AUDIO:v1:corrupt', conversationId1), isNull);
      expect(await service.decryptAudioBytes('ENC_AUDIO:v1:bad:data:format', conversationId1), isNull);
      expect(await service.decryptAudioBytes('', conversationId1), isNull);
      expect(await service.decryptAudio('ENC_AUDIO:v1:invalid:ciphertext:mac', conversationId1), isNull);
    });
  });
