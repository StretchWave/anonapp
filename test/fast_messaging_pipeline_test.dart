...
void main() {
  group('Fast Messaging & Cryptographic Key Cache Tests', () {
    ...
    test('EncryptionService caches derived conversation keys', () async {
      ...
      final encrypted1 = await EncryptionService.instance.encryptText('Hello World', convId);
      ...
      final decrypted1 = await EncryptionService.instance.decryptText(encrypted1, convId);
      expect(decrypted1, equals('Hello World'));
      expect(EncryptionService.instance.keyCacheSize, equals(1));

      // Another message in same conversation reuses key without increasing cache size
      final encrypted2 = await EncryptionService.instance.encryptText('Second Message', convId);
      expect(EncryptionService.instance.keyCacheSize, equals(1));
      final decrypted2 = await EncryptionService.instance.decryptText(encrypted2, convId);
      expect(decrypted2, equals('Second Message'));
    });

    test('EncryptionService separates keys across distinct conversation IDs', () async {
      ...
      final encAlpha = await EncryptionService.instance.encryptText('Secret Alpha', convId1);
      final failedDecrypt = await EncryptionService.instance.decryptText(encAlpha, convId2);
      expect(failedDecrypt, equals(encAlpha)); // Safe fallback returns raw ciphertext on key mismatch
    });
    ...
    test('clearKeyCache completely zeroizes cached keys', () async {
      ...
      final encrypted = await EncryptionService.instance.encryptText('Post Zeroize', convId);
      expect(EncryptionService.instance.keyCacheSize, equals(1));
      final decrypted = await EncryptionService.instance.decryptText(encrypted, convId);
      expect(decrypted, equals('Post Zeroize'));
    });
  });

  group('Dual-Track Broadcast & Reconciliation Logic Tests', () {
    test('Provisional broadcast message reconciles with server confirmed message by clientId', () {
      const convId = 'conv-1';
      ...
      final indexByClient = currentMessages.indexWhere(
        (m) =>
            (m.clientId != null && m.clientId == confirmedServerMsg.clientId) ||
